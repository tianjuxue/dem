...
batch_eval_sdf, batch_grad_sdf, get_rot_mats, batch_reference_to_physical, quat_mul
...
batch_compute_wall_reaction = jax.jit(jax.vmap(compute_wall_reaction, in_axes=(None, None, None, None, 0, 0), out_axes=0))


def compute_mutual_reaction(params, directions, connectivity, ref_centroid, x, q, index1, index2):
...
batch_compute_mutual_reaction = jax.jit(jax.vmap(compute_mutual_reaction, in_axes=(None,)*6 + (0,)*2, out_axes=0))
...
def state_rhs_func(params, directions, connectivity, state):
    ...
    mutual_distances = np.sqrt(np.sum(phy_origins**2, axis=1).reshape(-1, 1) + \
        np.sum(phy_origins**2, axis=1).reshape(1, -1) -  2 * np.dot(phy_origins, phy_origins.T))
    ...
    break1 = time.time()
    # Why this function gets recompiled during certain time step?
    reactions = batch_compute_mutual_reaction(params, directions, connectivity, ref_centroid, x.T, q.T, collision_indices[:, 0], collision_indices[:, 1])
    break2 = time.time()
    ...
    if time_elapesed > 1:
        print(f"------------------------------------------------------------------Recompile happens?{time_elapesed}")
